...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        if let window = window {
            ...
            dataController = DataController(startupCompletionClosure: {
                if let usersViewController = self.window?.rootViewController as? UsersViewController {
                    dispatch_async(dispatch_get_main_queue()) { // Updating the UI, so ensure it is on the main queue
                        usersViewController.setDataControllerAndUpdateUI(self.dataController)
                    }
                }
            })
        }
